// Simple working Socket.IO for Vercel
  ...
  if (!io) {
    ...
    io.on('connection', (socket) => {
      ...
      socket.on('webrtc:offer', (data) => {
        ...
        if (user) socket.to(user.meetingId).emit('webrtc:offer', { ...data, from: user.participantId });
      });

      socket.on('webrtc:answer', (data) => {
        const user = users.get(socket.id);
        if (user) socket.to(user.meetingId).emit('webrtc:answer', { ...data, from: user.participantId });
      });

      socket.on('webrtc:ice-candidate', (data) => {
        const user = users.get(socket.id);
        if (user) socket.to(user.meetingId).emit('webrtc:ice-candidate', { ...data, from: user.participantId });
      });
